...
/// Contains a message to be processed by a CloudAgent as well as a reply channel to confirm message completion.
type CloudMessage<'a> = 'a * (MessageProcessedStatus -> unit)

/// Represents an F# Agent that can be bound to an Azure Service Bus Queue for processing distributed messages with automatic retry.
type CloudAgent<'a> = MailboxProcessor<CloudMessage<'a>>
